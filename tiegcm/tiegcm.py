...
class TimeInterpolator(object):
	def __init__(self, interpolators):
		...
		try:
			t0, t1 = list(interpolators.keys())
		except:
			if len(list(interpolators.keys())) == 1:
				t0 = list(interpolators.keys())[0]
				t1 = t0
			else:
				print('could not construct TimeInterpolator with keys', end=' ')
				print(list(interpolators.keys()))
				raise
		self.t0 = t0
		self.t1 = t1
...
class TIEGCM(object):
	def __init__(self, filename, outermost_layer = -1):
		...
		self.set_variable_boundary_condition('ZG') #prior to wrapping to avoid double counting
		self.wrap_variable('ZG')
		self.z = np.array(self.rootgrp.variables['ZG'])[:,:self.outermost_layer,:,:] # Z is "geopotential height" -- use geometric height ZG?
		self.set_zmax()

		self.high_altitude_trees = dict()
		self.fill_value = np.nan

		self.midpoint_variables = ['TN', 'O2', 'O1', 'N2', 'HE', 'ZGMID', 'NO','CO2_COOL', 'NO_COOL', 'N2N'] # index with lev      
		self.interface_variables = ['DEN', 'ZG', 'Z']    #index with ilev

		try:
			self.set_variable_boundary_condition('ZGMID') #prior to wrapping to avoid double counting
			self.wrap_variable('ZGMID')
			self.zgmid = np.array(self.rootgrp.variables['ZGMID'])[:,:self.outermost_layer,:,:]         # zach added
			self.zgmid_scale = 100000
			self.UseMidpoints = True
		except:
			self.UseMidpoints = False
			pass
	...
	def get_outer_boundary_kdtree(self, time_index):
		...
		try:
			...
			outer_points = np.array(list(zip(z.ravel()/self.z_scale, lat.ravel(), lon.ravel())))
...
#					variable = fill_masked(variable)
	...
	def set_3d_boundary_conditions(self):
		...
		for k, variable in list(self.rootgrp.variables.items()):
	...
	def get_3D_column(self, column_slice, MidpointFlag):
		if self.UseMidpoints == True:
            
			if MidpointFlag == False:                             #zach added if statement
				z_column = self.z[column_slice] 
# 				print "z_column =", np.shape(z_column)
# 				print self.z.shape, column_slice
			else:
				z_column = self.zgmid[column_slice]         
# 				print "zgmid_column =", np.shape(z_column)
# 				print self.zgmid.shape, column_slice
		else:
			if MidpointFlag == False:                             #zach added if statement
				z_column = self.z[column_slice]
			else:
...
# 		z_column = self.z[column_slice] #4d
	...
	def get_delaunay_3D(self, z_column, lat_column, lon_column, time_index = 0):
		...
		"""
		points = list(zip(z_column[time_index].ravel()/self.z_scale, lat_column.ravel(), lon_column.ravel()))
	...
	def get_3D_interpolator(self, slice_key, time_index):
		if slice_key.variable in self.midpoint_variables:
			MidpointFlag = True
		else:
			MidpointFlag = False

# 		print 'Variable = ', slice_key.variable
# 		print slice_key.latitude, slice_key.longitude
        
		column_slice = ColumnSlice4D(*tuple(slice(*s[1]) for s in slice_key[:-1]))
		z_column, lat_column, lon_column = self.get_3D_column(column_slice, MidpointFlag)
		try:
		...
		try:
			...
			print('except @ get_3D_interpolator')
			print('\n', slice_key)
			print(column_slice)
	...
	def get_coordinate_indices(self, vertices, target_shape):
		return np.array(list(zip(*np.unravel_index(vertices, target_shape))))
	...
	def mass_density(self, point, time):
		...
		den_HE = self.molecular_mass_density(point, 'HE', time, density_tot)       #(ZACH added)
		if self.UseMidpoints == True:
			den_N2 = self.molecular_mass_density(point, 'N2', time, density_tot)   
		else:
...
# 		den_N2 = N2_n*m_p*28
# 		try:
# 		N2_n = self.time_interpolate(point, 'N2', time)
# 		den_N2 = N2_n*m_p*28
# 		except:
# 			N2_n = self.time_interpolate(point, 'N2N', time)
# 			den_N2 = N2_n*m_p*28         
# 		HE_n = self.time_interpolate(point, 'HE', time)                            # ZACH ADDED
# 		den_HE = HE_n*m_p*4                                                        # ZACH ADDED

		if self.UseMidpoints == False:
			h_bndy = self.time_interpolate(point, 'ZG', time)
		else:
			h_bndy = self.time_interpolate(point, 'ZGMID', time)
		...
		if point.height > h_bndy: 
			...
			h_HE = scale_height(T, point.height, 4)*100                             # ZACH ADDED  
			den_HE *= np.exp((h_bndy - point.height)/h_HE)                          # ZACH ADDED  

		rho = den_O1 + den_O2 + den_N2 + den_HE                                    # ZACH edited 
		return rho                                                                 
...
class Model_Manager():
	...
	"""
	def __init__(self, directory = None, outermost_layer = -4):
	...
	def get_file_for_time(self, time):
		for filename, interval in list(self.file_times.items()): 
	...
	def linear_time_interpolate(self, t0, t1, xlat, xlon, xalt, time):
		...
		if (t0,t1) in self.interpolators:
			...
			if file0 is None:
				...
				for k,v in list(self.file_times.items()):
...
# 	# z = tiegcm.z[time_index, -1, :, :] # topmost layer
	...
	p = Point3D(*p4[1:])
	result = tiegcm.time_interpolate_high_altitude(p, 'ZG', time)
